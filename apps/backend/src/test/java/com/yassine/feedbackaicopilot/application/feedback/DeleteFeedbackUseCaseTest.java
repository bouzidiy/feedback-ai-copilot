...
class DeleteFeedbackUseCaseTest {
    ...
    void shouldDeleteFeedback() {
        ...
        verify(feedbackRepositoryPort).deleteById(id);
    }

    @Test
    @DisplayName("Should reject delete when feedback does not exist")
    void shouldRejectDeleteWhenFeedbackDoesNotExist() {
        var id = UUID.randomUUID();

        when(feedbackRepositoryPort.findById(id)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> deleteFeedbackUseCase.delete(id))
